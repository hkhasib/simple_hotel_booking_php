...
if(isset($_SESSION['username'])){
    ...
    if($user_type=="admin" or $user_type=="staff"){
        $result3 = mysqli_query($conn,("Select book.*, hotel_rooms.*, room_type.* from book right outer join hotel_rooms on book.room_id=hotel_rooms.room_no right join room_type on hotel_rooms.room_code=room_type.room_code  order by book.from_date asc")) or die("Failed to Retrive Database");
        $result4 = mysqli_query($conn,("Select count(*) as total from book")) or die("Failed to Retrive Database");
    }
    else{
        $result3 = mysqli_query($conn,("Select book.*, hotel_rooms.*, room_type.* from book right outer join hotel_rooms on book.room_id=hotel_rooms.room_no right join room_type on hotel_rooms.room_code=room_type.room_code where md5(username)='$username' order by from_date asc")) or die("Failed to Retrive Database");
